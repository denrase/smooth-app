...
# so that tools like flutter/Android SDK are available.
if [[ -z "${_RUN_VALIDATION_LOGIN:-}" ]]; then
  export _RUN_VALIDATION_LOGIN=1
  exec zsh --login "$0" "$@"
fi
#
# Sentry Span-First Validation Runner — Android
#
# Runs Maestro flows against a connected Android device while capturing
# Flutter logs, then generates a validation report with auto-checks and
# Sentry links.
#
# Usage:
#   ./maestro/run_validation_android.sh                  # run all flows
#   ./maestro/run_validation_android.sh 02               # run only flow 02
#   ./maestro/run_validation_android.sh 02 03            # run flows 02 and 03
#
# Prerequisites:
#   - Android device connected via USB (adb devices)
...
#
# Output:
#   reports/run_android_YYYYMMDD_HHMMSS/
...
#     flutter_logs.txt        — raw logcat output
...
REPORT_DIR="$OUTPUT_DIR/run_android_${TIMESTAMP}"
...
cleanup() {
  for pid_var in BUILD_PID LOGCAT_PID; do
    local pid="${(P)pid_var:-}"
...
if ! command -v adb &>/dev/null; then missing+=("adb"); fi
...
# Prefer USB-connected device for ADB operations (build, install, logcat)
USB_DEVICE_ID=$(adb devices 2>/dev/null | awk 'NR>1 && $2=="device" && $1 !~ /:/ {print $1; exit}')

if [[ -z "$USB_DEVICE_ID" ]]; then
  echo "Error: No USB-connected Android device found."
  echo "  Connect a device via USB and ensure USB debugging is enabled."
  echo "  Check with: adb devices"
  exit 1
fi

DEVICE_MODEL=$(adb -s "$USB_DEVICE_ID" shell getprop ro.product.model 2>/dev/null | tr -d '\r')
DEVICE_BRAND=$(adb -s "$USB_DEVICE_ID" shell getprop ro.product.brand 2>/dev/null | tr -d '\r')
DEVICE_ANDROID=$(adb -s "$USB_DEVICE_ID" shell getprop ro.build.version.release 2>/dev/null | tr -d '\r')
DEVICE_SDK=$(adb -s "$USB_DEVICE_ID" shell getprop ro.build.version.sdk 2>/dev/null | tr -d '\r')

# Maestro's dadb library can't install APKs over USB on physical devices.
# Work around by switching to TCP/IP mode and using the network address.
info "Switching device to TCP/IP mode for Maestro compatibility..."
adb -s "$USB_DEVICE_ID" tcpip 5555 >/dev/null 2>&1
sleep 3
DEVICE_IP=$(adb -s "$USB_DEVICE_ID" shell ip route 2>/dev/null | awk '/wlan0/ {print $9}' | head -1)
if [[ -z "$DEVICE_IP" ]]; then
  echo "Error: Could not determine device IP. Is the device connected to Wi-Fi?"
  exit 1
fi
adb connect "${DEVICE_IP}:5555" >/dev/null 2>&1
sleep 2

# Use USB ID for adb operations, TCP ID for Maestro
DEVICE_ID="$USB_DEVICE_ID"
MAESTRO_DEVICE_ID="${DEVICE_IP}:5555"

info "Using device: $DEVICE_ID ($DEVICE_BRAND $DEVICE_MODEL, Android $DEVICE_ANDROID, API $DEVICE_SDK)"
info "Maestro will connect via: $MAESTRO_DEVICE_ID"

# ─── Clean install app ────────────────────────────────────────────────────

app_installed() {
  adb -s "$DEVICE_ID" shell pm list packages 2>/dev/null | grep -q "$APP_ID"
}
...
install_app() {
  ...
  if app_installed; then
    ...
    adb -s "$DEVICE_ID" uninstall "$APP_ID" 2>/dev/null || true
  ...
  (cd "$FLUTTER_PROJECT" && flutter run -t "$ENTRYPOINT" -d "$DEVICE_ID" 2>&1 \
    | while IFS= read -r line; do echo "  │ $line"; done) < /dev/null &
  BUILD_PID=$!

  # Wait for the app to be installed (up to 8 minutes — Android builds are slower)
  waited=0
  while ! app_installed; do
    if [[ $waited -ge 480 ]]; then
      kill "$BUILD_PID" 2>/dev/null || true
      wait "$BUILD_PID" 2>/dev/null || true
      echo "Error: Timed out waiting for app to install (8 min)."
      echo "  Try running manually: cd packages/smooth_app && flutter run -t $ENTRYPOINT -d $DEVICE_ID"
      exit 1
    fi
...
# Approve app links so openLink deep links go to the app (not Chrome)
info "Approving app links for deep link handling..."
adb -s "$DEVICE_ID" shell pm set-app-links --package "$APP_ID" 2 all 2>/dev/null || true
ok "App links approved"
echo ""
...
# Start log capture in background via adb logcat
# Use the TCP device for logcat — USB can become unreliable after tcpip switch
# Clear existing logcat buffer first for a clean capture
info "Starting log capture..."
adb -s "$MAESTRO_DEVICE_ID" logcat -c 2>/dev/null || true
adb -s "$MAESTRO_DEVICE_ID" logcat -v threadtime 'flutter:V' '*:S' \
  > "$LOG_FILE" 2>&1 &
LOGCAT_PID=$!
sleep 2

if ! kill -0 "$LOGCAT_PID" 2>/dev/null; then
  echo "Error: logcat failed to start. Is a device connected?"
  exit 1
fi

info "Log capture running (PID: $LOGCAT_PID)"
echo ""
...
echo "# Captured Spans"
echo ""
echo "**Run:** $TIMESTAMP | **Platform:** Android ($DEVICE_BRAND $DEVICE_MODEL, API $DEVICE_SDK) | **Total:** $TOTAL_SPANS | **Traces:** ${#UNIQUE_TRACES}"
echo ""
...
{
echo "# Sentry Span-First Validation Report — Android"
echo ""
echo "**Run:** $TIMESTAMP"
echo "**Device:** $DEVICE_BRAND $DEVICE_MODEL (Android $DEVICE_ANDROID, API $DEVICE_SDK)"
...
echo "| [flutter_logs.txt](flutter_logs.txt) | Raw logcat output |"
echo ""
...
echo "═══════════════════════════════════════════════════"
echo " Platform: Android ($DEVICE_BRAND $DEVICE_MODEL, API $DEVICE_SDK)"
